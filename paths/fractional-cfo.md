...
- **The client asks where the software budget went, or the cloud bill keeps growing** → [Software spend review](../skills/software-spend-review/) turns a bank or card export into a spend structure that holds. It splits the composite vendors before counting anything, because one cloud line is usually four costs (seats, usage, tokens, resold third-party software) sitting in two different places in the P&L, asks for the billing detail instead of guessing an allocation, and reports what it cannot resolve rather than dropping it in "other software".
